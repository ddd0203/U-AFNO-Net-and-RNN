...
def load_data(batch_size, val_batch_size, data_root, num_workers, dataname='custom', preload=True):
    ...
    train_set = GetDataset(train_data_root, preload=preload)
    valid_set = GetDataset(valid_data_root, preload=preload)
    test_set = GetDataset(test_data_root, preload=preload)

    # 优化：添加 persistent_workers 和 prefetch_factor
    dataloader_train = torch.utils.data.DataLoader(
        train_set, batch_size=batch_size, shuffle=True, pin_memory=True, 
        num_workers=num_workers, persistent_workers=num_workers > 0, prefetch_factor=2 if num_workers > 0 else None)
    dataloader_validation = torch.utils.data.DataLoader(
        valid_set, batch_size=val_batch_size, shuffle=False, pin_memory=True, 
        num_workers=num_workers, persistent_workers=num_workers > 0, prefetch_factor=2 if num_workers > 0 else None)
    dataloader_test = torch.utils.data.DataLoader(
        test_set, batch_size=val_batch_size, shuffle=False, pin_memory=True, 
        num_workers=num_workers, persistent_workers=num_workers > 0, prefetch_factor=2 if num_workers > 0 else None)
...
class GetDataset(Dataset):
    def __init__(self, location, data_name='custom', n_step_finetune=False, n_steps=2, preload=True):
    ...
    def _preload_all_data(self):
        """预加载所有数据到内存"""
        print(f"Preloading data from {self.location}...")
        self.data_cache = []
        for year_idx, file_path in enumerate(self.files_paths):
    ...
    def __getitem__(self, global_idx):
        ...
        if self.preload and self.data_cache is not None:
            fields_data = self.data_cache[year_idx]
        else:
            if self.files[year_idx] is None:
                self._open_file(year_idx)
            fields_data = self.files[year_idx]['fields']
        ...
        if self.n_step_finetune:
            ...
            inp = reshape_fields(fields_data[:, (local_idx - self.dt * self.n_history):(local_idx + 1):self.dt])
            ...
            for idx in target_indices:
                targets.append(fields_data[:, idx])
            ...
            inp = reshape_fields(fields_data[:, (local_idx - self.dt * self.n_history):(local_idx + 1):self.dt])
            tar = reshape_fields(fields_data[:, local_idx + step])
        return inp, tar
...
def test_dataloader():
    ...
    try:
        ...
        # 3.测试DataLoader (使用优化参数)
        print("测试DataLoader...")
        train_loader, val_loader, test_loader = load_data(
            batch_size=16, 
            val_batch_size=16,
            data_root=data_root,
            num_workers=4,  # 增加 workers
            dataname="custom",
            preload=True  # 预加载数据
        )
